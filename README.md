...
現時点では `Step 9` まで進み（全ステップ完了）、Django / PostgreSQL / Docker Compose の土台、ポータル認証引継ぎを受けるための `accounts` 基盤、資産台帳の `assets` 基盤、資産一覧 / 詳細の参照導線、貸出申請フロー、承認・返却フロー、インシデント管理、棚卸しセッション管理、監査ログ、デモシードデータ、単体テスト補完、URL / View 一覧、デモ実演手順、カバレッジ確認まで整備済みです。主要な方向性は `doc/spec/it-asset-management-demo.md` を基準にしています。
...
このディレクトリは、IT資産管理デモの仕様整理と実装を進めるための場所です。現在は Step 9 まで進み（全ステップ完了）、Django プロジェクト本体、認証受け皿、資産台帳の中核モデルと Admin 基盤、利用者向け / 管理者向けの資産参照導線、貸出申請フロー、承認・返却フロー、故障 / 紛失 / 廃棄のインシデント管理、棚卸しセッションと差異確認、監査ログ一覧、デモシード投入コマンド、主要テスト補完、ドキュメント整備、カバレッジ確認まで整備しています。
...
- URL / View 一覧: `doc/spec/url-view-list.md`
- デモ実演手順: `doc/spec/demo-procedure.md`
