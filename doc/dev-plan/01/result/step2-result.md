...
- 追加の単体テストとして `sys\app\accounts\tests\test_auth.py` を追加し、handover、未認証、logout、`returnTo` 制御を確認できるようにした
...
- `sys\app\accounts\tests\test_auth.py`
...
8. `docker compose --env-file sys\.env -f sys\infra\compose\docker-compose.yml run --rm --entrypoint python tdev-demo03-web manage.py test`
...
- `manage.py test` で handover、未認証、logout、`returnTo` 制御を含む単体テストが成功した
