...
`AUTH_MODE=dev-header` の環境では、画面上部のヘッダーに切替ウィジェットが表示されます。
- 現在ログイン中のユーザー名が表示されます（ハイライトされたボタン）
- 別のデモユーザーのボタンをクリックすると即座に切り替わります
- シードデータ未投入の場合はボタンが表示されません（`load_demo_seed` を先に実行してください）
